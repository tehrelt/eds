...
User::User(char* content)
{
	...
	std::memcpy(_hash_password, content + 4 + 16, 64);
}
...
User::User(int id, std::string name, std::string hash_password)
{
	...
	strncpy(_hash_password, hash_password.c_str(),	64);
}
...
char* User::ToChar()
{
	...
	std::memcpy(content + 4 + 16, _hash_password, 64);
